...
const testimonials = [
  ...
  {
    name: "Karthik S",
    text: " My problem was, I always used to enter late and miss the main move. Now using Intraday boost, able to spot good trades very early. Keep it up TradeFinder for making trading finding easy Mahesh Sir earlier I was doing only swing trading and positional trading but now after getting your TradeFinder able to trade confidently in intraday. Long way to go, but now I am confident.",
    image: userImg,
    hash: "#tradingmadesimple",
  },
];
...
const TestimonialsCarousel = () => {
  return (
    <div className="relative flex space-x-6 overflow-hidden h-[700px] ">
      {/* Blurred Overlay - Top */}
      <div className="absolute top-0 left-0 w-full h-40 bg-gradient-to-b from-[#0E1225] via-[#0E1225]/80 to-transparent z-10 pointer-events-none" />

      {/* Blurred Overlay - Bottom */}
      <div className="absolute bottom-0 left-0 w-full h-40 bg-gradient-to-t from-[#0E1225] via-[#0E1225]/80 to-transparent z-10 pointer-events-none" />
...
const Column = ({ testimonials, direction, speed }) => {
  ...
  return (
    <div className="w-1/3 h-[700px] overflow-hidden relative">
      <motion.div
      ...
      >
        {optimizedTestimonials.map((testimonial, index) => (
          <div
            key={index}
            className="bg-[#161B32] text-white p-4 rounded-lg shadow-md"
          >
            ...
            <p className="text-sm mt-5">{testimonial.hash}</p>
          </div>
        ))}
      </motion.div>
    </div>
